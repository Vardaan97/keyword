...
export async function saveResearchSession(session: Partial<DbResearchSession>): Promise<DbResearchSession | null> {
  if (!supabase) {
...
export async function getResearchSession(id: string): Promise<DbResearchSession | null> {
  if (!supabase) {
...
export async function getAllResearchSessions(limit = 50): Promise<DbResearchSession[]> {
  if (!supabase) {
...
export async function getCachedKeywords(cacheKey: string): Promise<KeywordIdeaDb[] | null> {
  if (!supabase) {
    return null
  }
...
export async function setCachedKeywords(cacheKey: string, keywords: KeywordIdeaDb[], ttlHours = 48): Promise<void> {
  if (!supabase) {
    return
  }
